...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: logs } = await supabase.from("abc_logs").select("*").eq("client_id", client_id).order("logged_at", { ascending: false }).limit(500);
    const { data: freq } = await supabase.from("teacher_frequency_entries").select("*").eq("client_id", client_id).eq("agency_id", agency_id).order("logged_date", { ascending: false }).limit(200);
    const { data: dur } = await supabase.from("teacher_duration_entries").select("*").eq("client_id", client_id).eq("agency_id", agency_id).order("logged_date", { ascending: false }).limit(200);
    const { data: categories } = await supabase.from("behavior_categories").select("*").eq("client_id", client_id);
    const { data: notes } = await supabase.from("teacher_quick_notes").select("*").eq("client_id", client_id).order("logged_at", { ascending: false }).limit(100);
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `Generate an FBA draft for student "${client_name || "Student"}".\n\nHere is all the collected behavioral data:\n${dataContext}` },
        ],
      }),
    });
