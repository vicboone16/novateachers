// IEP Document Processing Pipeline - Edge Function
...
serve(async (req) => {
  ...
  try {
    // Auth — users authenticate via Nova Core, so we just verify a Bearer token is present
    // and use service role key for DB operations on Lovable Cloud tables
