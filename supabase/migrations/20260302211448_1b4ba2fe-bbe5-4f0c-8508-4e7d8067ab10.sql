...
-- Fix 1: Replace overly permissive invite codes SELECT policy
-- Only code creators can view their own codes (agency_memberships is on external Core)
DROP POLICY IF EXISTS "Users can read active invite codes" ON public.agency_invite_codes;
...
-- Fix 2: Harden create_invite_code - use auth.uid() instead of trusting p_created_by
CREATE OR REPLACE FUNCTION public.create_invite_code(
  p_agency_id uuid,
  p_invite_scope text DEFAULT 'agency'::text,
  p_role_slug text DEFAULT 'teacher'::text,
  p_app_context text DEFAULT 'novatrack_teacher'::text,
  p_max_uses integer DEFAULT 10,
  p_expires_at timestamp with time zone DEFAULT NULL::timestamp with time zone,
  p_created_by uuid DEFAULT NULL::uuid,
  p_target_email text DEFAULT NULL::text,
  p_group_id uuid DEFAULT NULL::uuid,
  p_client_id uuid DEFAULT NULL::uuid,
  p_permissions jsonb DEFAULT NULL::jsonb,
  p_auto_assign_groups uuid[] DEFAULT NULL::uuid[]
)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_caller_id uuid;
BEGIN
  -- Always use authenticated user, never trust client-provided p_created_by
