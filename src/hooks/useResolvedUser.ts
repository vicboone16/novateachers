...
export function useResolvedUser(): ResolvedUserState {
  ...
  const fetchResolved = useCallback(async () => {
    ...
    if (import.meta.env.DEV) console.log('[useResolvedUser] Fetching for', user.email);
    const { data, error: err } = await resolveUser(
      user.email,
      session.access_token,
    );

    if (import.meta.env.DEV) console.log('[useResolvedUser] Result:', { data, error: err?.message });
    ...
    if (err) {
      ...
      if (import.meta.env.DEV) console.log('[useResolvedUser] agencies:', data.agencies?.length, data.agencies);
      setResolved(data);
      setCache(data);
    }
    setLoading(false);
