...
import { Star, Flag, Gift, Lock, Sparkles, Heart } from 'lucide-react';
...
export default function ParentSnapshot() {
  ...
  const loadSnapshot = async (t: string) => {
    ...
    try {
      ...
      if (new Date(tr.expires_at) < new Date()) {
        setError('This snapshot link has expired.');
        setLoading(false);
        return;
      }
      ...
      // Load current balance from Cloud ledger
      ...
      setError('Something went wrong loading this snapshot.');
    }
    setLoading(false);
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="text-center space-y-3 px-6">
          ...
          <p className="text-lg font-bold">{error || 'Unable to load snapshot'}</p>
          <p className="text-sm text-muted-foreground">Contact the teacher for a new link.</p>
        </div>
      </div>
    );
  }
  ...
  const formattedDate = new Date(data.snapshot_date + 'T12:00:00').toLocaleDateString(undefined, {
      ...
      <div className="mx-auto max-w-md px-4 py-8 space-y-5">
        ...
        <Card className="overflow-hidden border-0 shadow-lg">
          <CardContent className="p-5 text-center bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30">
            <div className="flex items-center justify-center gap-2 mb-1">
              ...
              <span className="text-3xl font-bold tabular-nums">{data.points_earned}</span>
            </div>
            <p className="text-sm text-muted-foreground">Points Earned Today</p>
            <p className="text-xs text-muted-foreground mt-1">Current balance: {balance} total</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/40">
          <CardContent className="p-4">
            ...
            <p className="text-[10px] text-muted-foreground mt-1 text-right">{Math.round(racePct)}% complete</p>
          </CardContent>
        </Card>
        ...
        {data.highlights.length > 0 && (
          <Card className="border-border/40">
            <CardContent className="p-4">
              <p className="text-sm font-bold mb-2">✨ Today's Highlights</p>
        ...
        {data.teacher_note && (
          <Card className="border-primary/20 bg-primary/5">
            <CardContent className="p-4">
              <p className="text-sm font-bold mb-1">📝 Teacher's Note</p>
              <p className="text-sm text-foreground/80">{data.teacher_note}</p>
            </CardContent>
          </Card>
        )}
