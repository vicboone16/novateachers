/**
 * RewardsPage — Dedicated Beacon Points & Rewards management page.
 * Shows: points balances, history, reward store, token boards.
 * Uses Core tables: beacon_points_ledger, beacon_rewards, beacon_reward_redemptions,
 * token_boards, reinforcement_templates, student_reinforcement_plans.
 */
...
import { Star, Gift, History, Trophy, Sparkles } from 'lucide-react';
...
const RewardsPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load point history from Cloud ledger
      const { data: histData } = await cloudSupabase
      ...
      // Get first group for token boards
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        ...
        <Card className="border-border/40">
          <CardContent className="p-3 text-center">
            <p className="text-2xl font-bold text-accent">{history.filter(h => h.points > 0).length}</p>
            <p className="text-[10px] text-muted-foreground">Awards Today</p>
          </CardContent>
        </Card>
        <Card className="border-border/40">
          <CardContent className="p-3 text-center">
            <p className="text-2xl font-bold text-destructive">{history.filter(h => h.points < 0).length}</p>
      ...
      <Tabs defaultValue="balances" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="store" className="gap-1.5 text-xs"><Gift className="h-3.5 w-3.5" /> Store</TabsTrigger>
              ...
              <Card key={c.id} className="border-border/40">
                <CardContent className="p-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-semibold">{displayName(c)}</p>
                      {c.grade && <p className="text-[10px] text-muted-foreground">Grade {c.grade}</p>}
                    </div>
                  </div>
                  <Badge className="gap-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-800">
                    ...
                    return (
                      <div key={h.id} className="flex items-center justify-between px-4 py-2.5">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Badge variant={h.points > 0 ? 'default' : 'destructive'} className="text-xs">
                            {h.points > 0 ? '+' : ''}{h.points}
                          </Badge>
                          <span className="text-[10px] text-muted-foreground">
                            {new Date(h.created_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}
                          </span>
                        </div>
                      </div>
                    );
